...
const getBackendUrl = () => {
  ...
  if (Platform.OS === 'web') {
    try {
      ...
      return 'https://mapyourworld.es:3000';
    } catch (error) {
  ...
  if (!expoUrl) {
    ...
    console.log("Usando IP estática como fallback: mapyourworld.es:3000");
    return "https://mapyourworld.es:3000";
  }
  ...
  try {
    ...
    if (!ip || !/^(\d{1,3}\.){3}\d{1,3}$/.test(ip)) {
      ...
      console.log("Usando IP estática como fallback: mapyourworld.es5:3000");
      return "https://mapyourworld.es:3000";
    }
...
//export const API_URL = process.env.API_URL || `https://52.143.134.165:3000`;
