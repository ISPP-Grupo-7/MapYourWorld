...
const getBackendUrl = () => {
  ...
  if (Platform.OS === 'web') {
    try {
      ...
      return 'https://localhost:3030'; // Cambia esto si tu servidor está en otro puerto
    } catch (error) {
  ...
  if (!expoUrl) {
    ...
    return "https://mapyourworld.es:3030";
  }
  ...
  try {
    ...
    return 'https://mapyourworld.es:3030';  // Cambia esto según tu entorno de Expo si es necesario
  } catch (error) {
