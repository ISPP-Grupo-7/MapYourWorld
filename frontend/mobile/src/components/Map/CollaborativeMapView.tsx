...
export const CollaborativeDistrictMap: React.FC<DistrictMapProps> = ({
  initialRegion,
  ...
  useEffect(() => {
    const getToken = async () => {
      try {
        ...
        console.error('No se pudo conseguir el token')
      }
    }
  ...
  // web socket
  const url = 'ws//localhost:8081' // TODO: change url once deployed
  const { connect, disconnect, subscribe, send } = token ? useSecureWebSocket({
    url: url,
    autoConnect: true,
    auth: { token: token }
  }) : useSecureWebSocket({
  ...
  const handleRegionChange = (newRegion: Region) => {
    ...
    // avisa al backend mediante el ws
    send(`map/${mapId}/region_discovered`,newRegion);
  };
  ...
  /**
   * Se conecta al web socket al iniciar la aplicación y se subscribe a los canales relevantes
   */
  const [mapId, setMapId] = useState<string | null>(null);
  useEffect(() => {
    // canales a los que se va a subscribir
    subscribe(`map/${mapId}`, handleMapUpdate);
  }, []);

  const handleMapUpdate = (updatedRegion : Region) => {
