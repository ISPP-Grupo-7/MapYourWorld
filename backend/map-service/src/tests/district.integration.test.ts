...
describe('District Service - Integration Tests', () => {
  ...
  describe('GET /api/districts/user/unlock', () => {
    it('debe obtener los distritos desbloqueados', async () => {
      mockDistrictRepo.find.mockResolvedValue([unlockedDistrict]);

      const response = await request(app)
        .get('/api/districts/user/unlock')
        .set('Authorization', `Bearer ${testToken}`);
  ...
  describe('GET /api/districts/user-districts/:userId', () => {
    it('debe obtener los distritos con colores asignados para un usuario', async () => {
      ...
      mockUserDistrictRepo
        .createQueryBuilder
        // Es importante simular también el comportamiento de la query
        .mockReturnValue({
